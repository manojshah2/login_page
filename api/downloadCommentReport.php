...
include_once($root."config/comfunc.php");
...
$date=getInputValue('start_date');
$enddate=getInputValue('end_date');


$final_query="SELECT Date(t1.`Date`) as Date,Time(t1.Date) as Time,  t1.`ADDED BY` as `CRM User`,t1.PID as `Profile Id`, t2.`First Name` , t2.`Last Name` ,t1.Comments , t2.`Data Taken From` ,t2.`client type` FROM `tblcomments` t1 left join tblprofiles t2 on t1.PID=t2.ID where Date(t1.`DATE`)>='".$date."' and Date(t1.`DATE`)<='".$enddate."'";
